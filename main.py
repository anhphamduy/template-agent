...
- "guidelines" MUST be a single English string summarizing overall test case generation guidelines derived from the template. If the template includes any matrix, decision tables, or parameter grids, include a dedicated part that: (1) lists the axes/fields, (2) enumerates allowed values or equivalence classes, (3) notes constraints and cross-field rules, (4) states the coverage strategy (e.g., pairwise, boundary, full Cartesian), and (5) provides step-by-step instructions to construct or reconstruct the matrix for the next test case suite (identify inputs/sources, derive equivalence classes, apply constraints, then produce the combinations). When quoting matrix/table names or values, preserve their original language.
